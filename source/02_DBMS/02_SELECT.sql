...
SELECT * FROM EMP; -- EMP테이블의 모든 열(필드) 을 출력해줌
SELECT * FROM DEPT; -- DEPT 테이블의 모든 열, 모든 행 출력
SELECT * FROM SALGRADE; -- SALGRADE 테이블 출력
-- 날짜 표기법 SETTING(YY/MM/DD 99/09/09)
...
SELECT EMPNO, ENAME, SAL, JOB FROM EMP;
    -- EMP 테이블 에서 EMPNO, ENAME , SAL, JOB 필드만 출력
SELECT EMPNO, ENAME, COMM FROM EMP;
SELECT EMPNO AS "사 번", ENAME AS "이름", SAL AS "급여" FROM EMP; -- 필드명에 별칭을 사용
SELECT EMPNO "사 번", ENAME "이름", SAL "급여" FROM EMP; --별칭에 AS 생략 가능 함
SELECT EMPNO NO, ENAME NAME, SAL SALARY FROM EMP; -- 별칭에 스페이스가 없는 경우 (") 생략가능
...
SELECT * FROM EMP WHERE SAL ^=3000;
    ...
    --EX1. 사원이름이 'A','B','C'로 시작하는 사원의 모든 필드를 출력하시오
SELECT * FROM EMP WHERE ENAME < 'D';
    --EX2. 81년도 이전에 입사한 사원의 모든 필드를 출력하시오
SELECT *FROM EMP WHERE HIREDATE < '81/01/01';

--4. 조건절에 논리연산자 : AND, OR, NOT 
    --EX1. 급여(SAL)가 2000이상 3000이하인 직원의 모들 필드를 출력하세요
SELECT * FROM EMP WHERE SAL >=2000 AND SAL<=3000;  -- JAVA 처럼 2000 <= SAL <=3000 이거 안됨
...
SELECT ENAME, SAL, SAL*12 "ANNUALSAL" --(3) 
    FROM EMP    --(1)  
    WHERE SAL*12>=2400 --(2) WHERE 절에는 필드의 별칭을 사용 불가
    ORDER BY ANNUALSAL; --(4) ORDER BY 절에는 별칭 사용 가능!

    --EX4. 10번부서(DEPTNO)이거나 JOB이 MANAGER인 직원의 모든 필드 출력 (OR 사용)
SELECT *FROM EMP WHERE DEPTNO=10 OR JOB='MANAGER';

    --EX5. 부서(DEPTNO) 10번이 아닌 직원 모든 필드 출력 (NOT 사용)
...
SELECT EMPNO, ENAME, JOB, DEPTNO FROM EMP WHERE NOT DEPTNO=20;
...
-- 8. SQL연산자 (BETWEEN, IN, LIKE, IS NULL) 연산자
    --(1) BETWEEN A AND B : A부터 B까지 // (A, B포함, A<B)
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL BETWEEN 1500 AND 3000;
    --EX. SAL이 1500미만, 3000초과 하는 직원의 모든 필드
...
--(2) IN 연산자 
    --EX. 부서번호가 10, 20, 40번 부서인 직원의 모든 필드 를 출력하시오    
SELECT * FROM EMP WHERE DEPTNO=10 OR DEPRNO=20 OR DEPTNO=40;
SELECT * FROM EMP WHERE DEPTNO IN (10,20,40);
    --EX. 부서번호가 10, 20, 40번을 제외한 부서의 직원들 모든 정보를 출력하시오
SELECT * FROM EMP WHERE DEPTNO NOT IN (10,20,40);
    --EX. 직책이 MANAGER이거나 ANALYST 인 사원의 모든 필드 를 출력하시오
SELECT * FROM EMP WHERE JOB IN ('MANAGER', 'ANALYST');

--(3) LIKE 연산자  ( 필드 > LIKE > 포맷 : %(0글자이상), _(한글자) 를 포함한 패턴)
...
SELECT * FROM EMP WHERE ENAME LIKE '%N%' OR JOB LIKE '%N%';
    --EX. 이름이 S로 끝나는 사원의 모든 필드 를 추력하시오
SELECT * FROM EMP WHERE ENAME LIKE '%S';
    --EX. SAL 이 5로 끝나는 사원의 모든 필드
SELECT * FROM EMP WHERE SAL LIKE '%5';
    --EX. 82년도 에 입사한 사원의 모든 필드
...
SELECT ENAME, SAL FROM EMP ORDER BY SAL; --(ORDER BY : 오름차순)
SELECT ENAME, SAL FROM EMP ORDER BY SAL DESC;--(뒤에 [DESC] 내림차순)
    --급여 내림차순 정렬 진행 ( 급여 가 같으면 입사 오름차순)
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL DESC, HIREDATE;    
    -- 급여 내림차순 정령 후 급여 같으면 신입순
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL DESC, HIREDATE DESC;

SELECT * FROM EMP ORDER BY ENAME; --ABC 순으로 정령
...
SELECT ENAME, JOB, HIREDATE FROM EMP WHERE HIREDATE BETWEEN TO_DATE('1981-02-20') AND TO_DATE('1981-05-01')
ORDER BY HIREDATE;
--5.	deptno가 10,20인 사원의 모든 정보를 출력 (단 ename순으로 정렬)
SELECT * FROM EMP WHERE DEPTNO IN (10,20) ORDER BY ENAME;
--6.	sal이 1500이상이고 deptno가 10,30인 사원의 ename과 sal를 출력
-- (단 HEADING(타이틀)을 employee과 Monthly Salary로 출력)
SELECT ENAME AS EMPLOYEE, SAL AS MONTHLYSALARY, DEPTNO FROM EMP WHERE SAL>=1500 AND DEPTNO IN(10,30);
--7.	hiredate가 1982년인 사원의 모든 정보를 출력
SELECT * FROM EMP WHERE HIREDARE LIKE '82/%'; 
...
SELECT * FROM EMP WHERE JOB  IN('CLERK','ANALTST') AND SAL NOT IN(1000,3000,5000);
...
SELECT EMPNO, ENAME, HIREDATE, JOB, SAL FROM EMP WHERE TO_CHAzR(HIREDATE, 'YYYY') = '1981';
